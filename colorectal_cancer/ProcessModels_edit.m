...
for s_loop = 1:2
    ...
    StageDistMatrix = S;
    for i=1:21 % i = 21 because we just have data columns for 21 region including south america
    ...
    %%
        ...
        DATA=RAW_D(ind,6);
        if s_loop==1
        pA=cell2mat(DATA(163:243))'; % population by age for female
        Mx=cell2mat(DATA(406:486))'; % natural mortality by age
        else
        pA=cell2mat(DATA(82:162))'; % population by age for male 
        Mx=cell2mat(DATA(325:405))'; % natural mortality by age
        end
...
%         if Countries{I} == 'Peru'
%             % female
%             if sex(s_loop) == 'Female'
%                 
%                 % data for DCO mentioned below is for the different regions
%                 % of Argentina (Percentage values)
%                 DCO = [10.3 19.8 15.1 3.3];
%                 avg = mean(DCO)/100;
%                 PAR{4} = avg*PAR{4};
%                 
%             % male
%             elseif sex(s_loop) == 'Male'
%                 
%                 % data for DCO mentioned below is for the different regions
%                 % of Argentina (Percentage values)
%                 DCO = [9.5 16.2 13.5 2];
...
'------- results are written for both sexes ---------'
...
function [lTEMPLATE]=WriteTemplate(RES,TEMPLATE)
    ...
    lTEMPLATE{ R{onset,1}+I_loop-1 ,R{onset,3}}=PolyponsetRateAge(I_loop,1); 

    %healthy to pre-clinical
    lTEMPLATE{ R{DF_PC1,1}+I_loop-1 ,R{DF_PC1,3}}=PolyponsetRateAge(I_loop,5);    
    ...
    lTEMPLATE{ R{PC4_CFR,1}+I_loop-1 ,R{PC4_CFR,3} }=MortRate(4,I_loop);
    ...
    lTEMPLATE{ R{I5_CFR_Function,1}+I_loop-1 ,R{I5_CFR_Function,3} }=Impact(4,I_loop);%INTVN{4,2}*100;
    lTEMPLATE{ R{I6_CFR_Function,1}+I_loop-1 ,R{I6_CFR_Function,3} }=Impact(4,I_loop);%INTVN{4,2}*100;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    lTEMPLATE{ R{Tx_C5,1}+I_loop-1 ,R{Tx_C5,4} }  = TxCoverage(4,I_loop)*100;
    lTEMPLATE{ R{Tx_C6,1}+I_loop-1 ,R{Tx_C6,4} }  = TxCoverage(4,I_loop)*100;
    lTEMPLATE{ R{Tx_C7,1}+I_loop-1 ,R{Tx_C7,4} }  = TxCoverage(4,I_loop)*100;
    lTEMPLATE{ R{Tx_C8,1}+I_loop-1 ,R{Tx_C8,4} }  = TxCoverage(4,I_loop)*100;
    ...
    lTEMPLATE{ R{Tx_C5,1}+I_loop-1 ,R{Tx_C5,5} }  = ((1-TxCoverage(4,I_loop))*.8+TxCoverage(4,I_loop)) *100;
    lTEMPLATE{ R{Tx_C6,1}+I_loop-1 ,R{Tx_C6,5} }  = ((1-TxCoverage(4,I_loop))*.8+TxCoverage(4,I_loop)) *100;
    lTEMPLATE{ R{Tx_C7,1}+I_loop-1 ,R{Tx_C7,5} }  = ((1-TxCoverage(4,I_loop))*.8+TxCoverage(4,I_loop)) *100;
    lTEMPLATE{ R{Tx_C8,1}+I_loop-1 ,R{Tx_C8,5} }  = ((1-TxCoverage(4,I_loop))*.8+TxCoverage(4,I_loop)) *100;
    ...
    lTEMPLATE{ R{Tx_C5,1}+I_loop-1 ,R{Tx_C5,6} }  = TxCoverage(4,I_loop)*100;
    lTEMPLATE{ R{Tx_C6,1}+I_loop-1 ,R{Tx_C6,6} }  = TxCoverage(4,I_loop)*100;
    ...
    lTEMPLATE{ R{Tx_C5,1}+I_loop-1 ,R{Tx_C5,7} }  = ((1-TxCoverage(4,I_loop))*.8+TxCoverage(4,I_loop)) *100;
    lTEMPLATE{ R{Tx_C6,1}+I_loop-1 ,R{Tx_C6,7} }  = ((1-TxCoverage(4,I_loop))*.8+TxCoverage(4,I_loop)) *100;
    ...
    lTEMPLATE{ R{I5_CFR_Function,1}+I_loop-1 ,R{I5_CFR_Function,4} }=-(DW{4,1}-DW{5,2})/DW{4,1}*100;
    lTEMPLATE{ R{I6_CFR_Function,1}+I_loop-1 ,R{I6_CFR_Function,4} }=-(DW{4,1}-DW{6,2})/DW{4,1}*100;

    %Palliative Care
    lTEMPLATE{ R{I7_Function,1}+I_loop-1 ,R{I7_Function,4} }=-(DW{4,1}-DW{5,2})/DW{4,1}*100;
    lTEMPLATE{ R{I8_Function,1}+I_loop-1 ,R{I8_Function,4} }=-(DW{4,1}-DW{6,2})/DW{4,1}*100;
    ...
    %Baseline Prevalence
    lTEMPLATE{ R{PCPolypLT5mm_Prev0,1}+I_loop-1 ,R{PCPolypLT5mm_Prev0,3} }=PrevPolys(I_loop,1);
    lTEMPLATE{ R{PCPolyp6to10mm_Prev0,1}+I_loop-1 ,R{PCPolyp6to10mm_Prev0,3} }=PrevPolys(I_loop,2);
    lTEMPLATE{ R{PCPolypGT10mm_Prev0,1}+I_loop-1 ,R{PCPolypGT10mm_Prev0,3} }=PrevPolys(I_loop,3);

    %CFR rate
    lTEMPLATE{ R{PCPolypLT5mm_CFR,1}+I_loop-1 ,R{PCPolypLT5mm_CFR,3} }=0;
    lTEMPLATE{ R{PCPolyp6to10mm_CFR,1}+I_loop-1 ,R{PCPolyp6to10mm_CFR,3} }=0;
    lTEMPLATE{ R{PCPolypGT10mm_CFR,1}+I_loop-1 ,R{PCPolypGT10mm_CFR,3} }=0;

    %Progression
    lTEMPLATE{ R{PCPolypLT5mm_Progr,1}+I_loop-1 ,R{PCPolypLT5mm_Progr,3} }=0.021;
    lTEMPLATE{ R{PCPolyp6to10mm_Progr,1}+I_loop-1 ,R{PCPolyp6to10mm_Progr,3} }=0.057;
    lTEMPLATE{ R{PCPolypGT10mm_Progr,1}+I_loop-1 ,R{PCPolypGT10mm_Progr,3} }=0.063;
