function [RES]=MarkovChainCountry2_edit(PAR, sex, country)
...
%% Altering the age distribution according to sex and country
    % Country-specific parameters
    
    % sex-ratio data taken from following link
    % https://www.indexmundi.com/peru/sex_ratio.html
%     male_dist = ones(1, length(PAR{1}));
%     male_dist(1:14) = (1.04)/(1 + 1.04);
%     male_dist(15:24) = (1)/(1 + 1);
%     male_dist(25:54) = (0.92)/(1 + 0.92);
%     male_dist(55:64) = (0.94)/(1 + 0.94);
%     male_dist(65:end) = (0.9)/(1 + 0.9);
%     
%     if (country == "Peru" && sex == "Female")
%         female_dist = ones(1, length(PAR{1})) - male_dist;
%         PAR{1} = PAR{1}.*female_dist;  
%     elseif (country == "Peru" && sex == "Male")
%         PAR{1} = PAR{1}.*male_dist; 
%     else
%         PAR{1} = 0.5*PAR{1};
%     end
...
%% Get polyp incidence in the format required by Word (manuscript)
[word_format, inci_val] = get_word_format(polypOnsetRateAgeGroup,PAR{1},AgeArrayLower,AgeArrayUpper);
    
%% calculation of diagnostic rates stars here  
    
    for upper_start = 1:1 % upper bound for the starting point
        trial_run = 1;
        
        % diagnostic rate matrix
        betaRuns = ones(4,size(AgeArrayLower,2),trial_run);
        
        % error matrix
        run_error = zeros(1, trial_run); 

        for randRuns = 1:trial_run
            beta0 = ones(4,size(AgeArrayLower,2)) * rand()*upper_start/10;
            ...
            tic
            [Z,PrevS,PrevC,diagrT,beta,err]=FitSoJourn(1000,beta0,X,Y,PAR,Incidence,OnsetRatesF,DwellTimesF,AgeArrayLower,AgeArrayUpper, Mu_Tx, BaseMortality);
            toc
            ...
            % array for starting points for algorithm
            initial_point_array(:,randRuns)=initial_point;
            % an array for total error for each staring consition
            run_error(1,randRuns) = err;
        ...
        % plotting error
        figure
        plot(initial_point_array(1,:),run_error);
        xlabel('Staring point for algorithm');
        ylabel('Error');
        title(' Error for each trial ');
        
        %% matrices for analysis/debug
        % collecting output in tractable format
        for i_l = 1:trial_run
            for j = 2:4
                stack((j-2)*13 + 1:(j-2)*13 + 13,i_l) = trial_details(:,j,i_l);
            end
        end
...
function [PrevA,PrevS,PrevC,diagr,beta,err]=FitSoJourn(MaxFunEvals,beta0,X,Y,PAR,Incidence,OnsetRatesF,DwellTimesF,AgeArrayLower,AgeArrayUpper, Mu_Tx, BaseMortality,  InsituOnsetRate)
    ...
    % if there are some diagnosed cases in 
    StageDist = PAR{6};
    if StageDist(1,1) == 0
        LB(1,:) = 0;
        UB(1,:) = 0;
...
%     beta0,X,Y,LB,UB,OPTIONS);
    ...
    %plot the results
    [PrevA,diagr,PrevS,PrevC,err]=PopSim(beta,X,PAR,Incidence,OnsetRatesF,DwellTimesF,AgeArrayLower,AgeArrayUpper,Mu_Tx, BaseMortality,1,10^5,store_beta,Y,state_c);

end

%% folowing function is not used for age-stage fitting
...
%% following function is not used for age-stage fitting
...
function [Z,diagr,PrevS,PrevC,err]=PopSim(fitPAR,X,PAR,Incidence,OnsetRatesF,DwellTimesF,AgeArrayLower,AgeArrayUpper, Mu_Tx, BaseMortality, PlotModel, age, store_beta,Y,state_c)
...
%%        
    ...
    pop0=10^5;%work with pop of standard size
...
%% work under progress (START)
    
%     %% Creating rate matrix and doing pop sim without equation
%     
%     % creating rate matrix
%     q_rate = zeros(10,10,101);
%     q_rate(1,2,:) = OnsetRatesF';
%     q_rate(1,10,:) = mu';
%     for i = 2:10
%         q_rate(i,10,:) = mu';
%         if i<=5
%             q_rate(i,i+1,:) = lambda(:,i-1)';
%             q_rate(i,i+4,:) = diagrT(:,i-1)';
%         end
%     end
% 
%     for i = 1:101
%         diag_mat = diag(q_rate(:,:,i));
%         sum_rate = -1*(sum(q_rate(:,:,i),2) - diag_mat(:,:,1));
%         for j=1:10
%             q_rate(j,j,i) = sum_rate(j,1);
%         end
%         q_rate(10,1,i) = 1;
%         q_rate(10,10,i) = -1;
%     end
...
%% work under progress (END)        
    
%% POP sim with equations
    dt=1/15;
    for t=1:t_max
        ...
        for t1=1:1/dt    
            ...
            % original
            POP(:,2)=POP(:,2)+dt*(-mu-lambda(:,1)-diagr(:,1)).*POP(:,2) + dt*(OnsetRatesF).*POP(:,1);   % pre-clinical-1
            POP(:,3)=POP(:,3)+dt*(-mu-lambda(:,2)-diagr(:,2)).*POP(:,3) + dt*(lambda(:,1)).*POP(:,2);   % PC-2
            POP(:,4)=POP(:,4)+dt*(-mu-lambda(:,3)-diagr(:,3)).*POP(:,4) + dt*(lambda(:,2)).*POP(:,3);   % PC-3
            POP(:,5)=POP(:,5)+dt*(-mu-lambda(:,4)-diagr(:,4)).*POP(:,5) + dt*(lambda(:,3)).*POP(:,4);   % PC-4
        ...
        Births=BirthsF*10^4;%BirthsF*sum(POP(:));
        POP(1,1)=Births;
    ...
    %% plotting and calculating incidence fit
