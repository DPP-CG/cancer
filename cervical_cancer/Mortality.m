...
%Goldie et al.,2003 [9], NCCN, 2016 [10], Chuang, 2016 [11]
%Fo Stage 0, assuming 1/100 considering maximum life of 100
RNoTx = [0.001 0.120	0.196	0.4766	1.266];
RTx = [0.001 0.027 0.062 0.167 0.316];

MaxAge = 101;
maxStage = size(StageDist,2);
RTxByNoTx = RTx./RNoTx;
for i = 1:MaxAge
StgDist(i,:) =  StageDist;
end
...
% Ten-year survival without treatment
   ...
   Healthy(1,i) = TenYearSurvival(MortDFbyAge(1,i:i+9),zeros(1,10));
   Insitu(1,i) = TenYearSurvival(DwellRate(1:stg,i:i+9),MortDFbyAge(1,i:i+9));
   Local(1,i) = TenYearSurvival(DwellRate(2:stg,i:i+9),MortDFbyAge(1,i:i+9));
   Regional(1,i) = TenYearSurvival(DwellRate(3:stg,i:i+9),MortDFbyAge(1,i:i+9));
   Distant(1,i) = TenYearSurvival(DwellRate(4:stg,i:i+9),MortDFbyAge(1,i:i+9));
   StageIV(1,i) = TenYearSurvival(DwellRate(5:5,i:i+9),MortDFbyAge(1,i:i+9));
...
% Ten-year RELATIVE survival without treatment   
  ...
  %Yearly mortality rate without treatment 
   ...
   if StageDist(1,5) == 0
    ...
    Mu_NoTx(5,i) = (MortDFbyAge(1,i)*t - log(RRStageIV(1,i)))/t;
   end
end
...
for stage = 1:maxStage
    Mu_Tx(stage,:) = max(0,Mu_Tx(stage,:) - MortDFbyAge(1,:));
    Mu_NoTx(stage,:) = max(0,Mu_NoTx(stage,:) - MortDFbyAge(1,:));
    RMortalityTx(stage,:)= Mu_Tx(stage,:)./ MortDFbyAge(1,:);
    RMortalityNoTx(stage,:)= Mu_NoTx(stage,:)./ MortDFbyAge(1,:);
end    
    
    
Coverage = ones(MaxAge, maxStage)*1;
BaseMortality = zeros(size(Mu_Tx,1), size(Mu_Tx,2));

for stage = 1:maxStage
    %Basemortality overall mortality in diagnsoed disease state weighted
    %by treatmnet covergae
    BaseMortality(stage,:) = (Coverage(:,stage)' .*  Mu_Tx(stage,:) + (1- Coverage(:,stage)').* Mu_NoTx(stage,:))  ;
    % estimating increase in weighted mortality for unit change in coverage
    Impact (stage,:) = (-Mu_Tx(stage,:) + Mu_NoTx(stage,:)) ./ BaseMortality(stage,:);
end 

TxCoverage(:,:) = max(0,Coverage(:,1:maxStage));
