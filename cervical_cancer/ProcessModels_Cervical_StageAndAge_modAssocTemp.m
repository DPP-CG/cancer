function ProcessModels_Cervical_StageAndAge

[~,~,TEMPLATEW]=xlsread('CervicalCancer_AssoTemplate.xlsx','Female');
[~,~,TEMPLATEM]=xlsread('CervicalCancer_AssoTemplate.xlsx','Male');
...
% cancer data
[~,Countries] = xlsread('DataInci.xlsx','B3:R3');
[StageDistMatrix] = xlsread('DataInci.xlsx','B32:R36'); 
[prevalenceActualMatrix] = xlsread('DataInci.xlsx','B4:R13');
[distHPV] = xlsread('DataInci.xlsx','B42:R43');
[distHigh] = xlsread('DataInci.xlsx','B48:R49');
[cancerMortalityArrayMatrix] = xlsread('DataInci.xlsx','B18:R27');
...
%% Parameters are now fed to the function which calculates the onset rate and diagnostic rates
    [RES,RES2]=MarkovChainCountry2_StageAndAge(PAR);
...
%     
    ...
    xlswrite(str,C_TEMPLATE,'Female');
...
% for I=1:length(Countries)
  ...
  %  if I == 16
        ...
        [~,~,data_asso] = xlsread('CervicalCancer_AssoTemplate','Transmission','A1:M44');
        ...
        Transmission(1,1) = cellstr('Countries{I}'); Transmission(1,2:end) = num2cell(NaN);
...
function [lTEMPLATE]=WriteTemplateW(RES,TEMPLATE)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 StateIDColIndex = 3; %location of STateID in Template
 FirstDataRowIndex = 5;%number of rows below StateID that data starts

    %1=row 1, 2=row end, 3=row col
    %disease free
    StateIDRowIndex = findRow('DsFreeSus', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ;  
    R{onset,1}=StateIDRowIndex;             R{onset,3}=7;
    R{DSF1_inc1618,1}=StateIDRowIndex;      R{DSF1_inc1618,3}=7;
    R{DSF1_incH,1}=StateIDRowIndex;         R{DSF1_incH,3}=10;
    R{DSF1_incL,1}=StateIDRowIndex;         R{DSF1_incL,3}=13;
    
    %Vaccinated
    StateIDRowIndex = findRow('Vaccinated', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ;  
    R{Vacc_incH,1}=StateIDRowIndex;         R{Vacc_incH,3}=6;
    R{Vacc_incL,1}=StateIDRowIndex;         R{Vacc_incL,3}=9;
    
    %pre-cancer stages
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %HPV 16/18
    StateIDRowIndex = findRow('HPV16_18', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ; 
    R{PCH1_1618_Prev0,1}=StateIDRowIndex;       R{PCH1_1618_Prev0,3}=4;
    R{PCH1_1618_CFR,1}=StateIDRowIndex;       R{PCH1_1618_CFR,3}=6;
    R{PCH1_1618_Rem,1}=StateIDRowIndex;       R{PCH1_1618_Rem,3}=9;
    R{PCH1_1618_Progr,1}=StateIDRowIndex;       R{PCH1_1618_Progr,3}=12;

    %CIN1 16/18
    StateIDRowIndex = findRow('CIN1_16_18', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ; 
    R{PCH2_1618_Prev0,1}=StateIDRowIndex;       R{PCH2_1618_Prev0,3}=4;
    R{PCH2_1618_CFR,1}=StateIDRowIndex;       R{PCH2_1618_CFR,3}=6;
    R{PCH2_1618_Rem1,1}=StateIDRowIndex;       R{PCH2_1618_Rem1,3}=9;
    R{PCH2_1618_Rem2,1}=StateIDRowIndex;       R{PCH2_1618_Rem2,3}=12;
    R{PCH2_1618_Progr,1}=StateIDRowIndex;       R{PCH2_1618_Progr,3}=15;

    %CIN2/3 16/18
    StateIDRowIndex = findRow('CIN2_3_16_18', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ; 
    R{PCH3_1618_Prev0,1}=StateIDRowIndex;       R{PCH3_1618_Prev0,3}=4;
    R{PCH3_1618_CFR,1}=StateIDRowIndex;       R{PCH3_1618_CFR,3}=6;
    R{PCH3_1618_Rem1,1}=StateIDRowIndex;       R{PCH3_1618_Rem1,3}=9;
    R{PCH3_1618_Rem2,1}=StateIDRowIndex;       R{PCH3_1618_Rem2,3}=12;
    R{PCH3_1618_Progr,1}=StateIDRowIndex;       R{PCH3_1618_Progr,3}=15;

    %HPV High Risk
    StateIDRowIndex = findRow('HPVHighRisk', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ; 
    R{PCH1_H_Prev0,1}=StateIDRowIndex;       R{PCH1_H_Prev0,3}=4;
    R{PCH1_H_CFR,1}=StateIDRowIndex;       R{PCH1_H_CFR,3}=6;
    R{PCH1_H_Rem,1}=StateIDRowIndex;       R{PCH1_H_Rem,3}=9;
    R{PCH1_H_Progr,1}=StateIDRowIndex;       R{PCH1_H_Progr,3}=12;

    %CIN1 High Risk
    StateIDRowIndex = findRow('CIN1HighRisk', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ; 
    R{PCH2_H_Prev0,1}=StateIDRowIndex;       R{PCH2_H_Prev0,3}=4;
    R{PCH2_H_CFR,1}=StateIDRowIndex;       R{PCH2_H_CFR,3}=6;
    R{PCH2_H_Rem1,1}=StateIDRowIndex;       R{PCH2_H_Rem1,3}=9;
    R{PCH2_H_Rem2,1}=StateIDRowIndex;       R{PCH2_H_Rem2,3}=12;
    R{PCH2_H_Progr,1}=StateIDRowIndex;       R{PCH2_H_Progr,3}=15;

    %CIN2/3 High Risk
    StateIDRowIndex = findRow('CIN2_3HighRisk', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ; 
    R{PCH3_H_Prev0,1}=StateIDRowIndex;       R{PCH3_H_Prev0,3}=4;
    R{PCH3_H_CFR,1}=StateIDRowIndex;       R{PCH3_H_CFR,3}=6;
    R{PCH3_H_Rem1,1}=StateIDRowIndex;       R{PCH3_H_Rem1,3}=9;
    R{PCH3_H_Rem2,1}=StateIDRowIndex;       R{PCH3_H_Rem2,3}=12;
    R{PCH3_H_Progr,1}=StateIDRowIndex;       ;R{PCH3_H_Progr,3}=15;

    %HPV Low Risk
    StateIDRowIndex = findRow('HPVLowRisk', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ; 
    R{PCL1_Prev0,1}=StateIDRowIndex;       R{PCL1_Prev0,3}=4;
    R{PCL1_CFR,1}=StateIDRowIndex;       R{PCL1_CFR,3}=6;
    R{PCL1_Rem,1}=StateIDRowIndex;       R{PCL1_Rem,3}=9;
    R{PCL1_Progr,1}=StateIDRowIndex;       R{PCL1_Progr,3}=12;

    %CIN1 Low Risk
    StateIDRowIndex = findRow('CIN1LowRisk', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ; 
    R{PCL2_Prev0,1}=StateIDRowIndex;       R{PCL2_Prev0,3}=4;
    R{PCL2_CFR,1}=StateIDRowIndex;       R{PCL2_CFR,3}=6;
    R{PCL2_Rem1,1}=StateIDRowIndex;       R{PCL2_Rem1,3}=9;
    R{PCL2_Rem2,1}=StateIDRowIndex;       R{PCL2_Rem2,3}=12;
    R{PCL2_Progr,1}=StateIDRowIndex;       R{PCL2_Progr,3}=15;

    %CIN2/3 Low Risk
    StateIDRowIndex = findRow('CIN2_3Low_Risk', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ; 
    R{PCL3_Prev0,1}=StateIDRowIndex;       R{PCL3_Prev0,3}=4;
    R{PCL3_CFR,1}=StateIDRowIndex;       R{PCL3_CFR,3}=6;
    R{PCL3_Rem1,1}=StateIDRowIndex;       R{PCL3_Rem1,3}=9;
    R{PCL3_Rem2,1}=StateIDRowIndex;       R{PCL3_Rem2,3}=12;
    R{PCL3_Progr,1}=StateIDRowIndex;       R{PCL3_Progr,3}=15;

    %HPV Regression
    StateIDRowIndex = findRow('HPV16_18Regression', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ; 
    R{PCR1_Prev0,1}=StateIDRowIndex;       R{PCR1_Prev0,3}=4;
    R{PCR1_CFR,1}=StateIDRowIndex;       R{PCR1_CFR,3}=6;
    R{PCR1_Rem,1}=StateIDRowIndex;       R{PCR1_Rem,3}=9;
   
    StateIDRowIndex = findRow('HPVHighRiskRegression', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ; 
    R{PCR2_Prev0,1}=StateIDRowIndex;       R{PCR2_Prev0,3}=4;
    R{PCR2_CFR,1}=StateIDRowIndex;       R{PCR2_CFR,3}=6;
    R{PCR2_Rem,1}=StateIDRowIndex;       R{PCR2_Rem,3}=9;
   
    StateIDRowIndex = findRow('HPVLowRiskRegression', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ; 
    R{PCR3_Prev0,1}=StateIDRowIndex;       R{PCR3_Prev0,3}=4;
    R{PCR3_CFR,1}=StateIDRowIndex;       R{PCR3_CFR,3}=6;
    R{PCR3_Rem,1}=StateIDRowIndex;       R{PCR3_Rem,3}=9;




    %Immunity
    StateIDRowIndex = findRow('Immunity', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ; 
    R{PCM1_Prev0,1}=StateIDRowIndex;       R{PCM1_Prev0,3}=4;
    R{PCM1_CFR,1}=StateIDRowIndex;       R{PCM1_CFR,3}=6;
    R{PCM1_Progr,1}=StateIDRowIndex;       R{PCM1_Progr,3}=9;
    ...
    %pre clinical
    %stage 0
    StateIDRowIndex = findRow('PreClinical0', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ;  
    R{PC1_Prev0,1}=StateIDRowIndex;         R{PC1_Prev0,3}=4;
    R{PC1_CFR,1}=StateIDRowIndex;           R{PC1_CFR,3}=6;
    R{PC1_Screen,1}=StateIDRowIndex;        R{PC1_Screen,3}=9;
    R{PC1_Progr,1}=StateIDRowIndex;         R{PC1_Progr,3}=12;

    %stage 1
    StateIDRowIndex = findRow('PreClinical1', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ;  
    R{PC2_Prev0,1}=StateIDRowIndex;     R{PC2_Prev0,3}=4;
    R{PC2_CFR,1}=StateIDRowIndex;       R{PC2_CFR,3}=6;
    R{PC2_Screen,1}=StateIDRowIndex;    R{PC2_Screen,3}=9;
    R{PC2_Progr,1}=StateIDRowIndex;     R{PC2_Progr,3}=12;

    %stage 2
    StateIDRowIndex = findRow('PreClinical2', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ;  
    R{PC3_Prev0,1}=StateIDRowIndex;     R{PC3_Prev0,3}=4;
    R{PC3_CFR,1}=StateIDRowIndex;     R{PC3_CFR,3}=6;
    R{PC3_Screen,1}=StateIDRowIndex;     R{PC3_Screen,3}=9;
    R{PC3_Progr,1}=StateIDRowIndex;     R{PC3_Progr,3}=12;

    %stage 3
    StateIDRowIndex = findRow('PreClinical3', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ;  
    R{PC4_Prev0,1}=StateIDRowIndex;     R{PC4_Prev0,3}=4;
    R{PC4_CFR,1}=StateIDRowIndex;     R{PC4_CFR,3}=6;
    R{PC4_Screen,1}=StateIDRowIndex;     R{PC4_Screen,3}=9;
    R{PC4_Progr,1}=StateIDRowIndex;     R{PC4_Progr,3}=12;  

    %stage 4
    StateIDRowIndex = findRow('PreClinical4', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ;
    R{PC5_Prev0,1}= StateIDRowIndex;     R{PC5_Prev0,3}=4;
    R{PC5_CFR,1}=StateIDRowIndex;     R{PC5_CFR,3}=6;
    R{PC5_Screen,1}=StateIDRowIndex;     R{PC5_Screen,3}=9;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %clinical 0
    StateIDRowIndex = findRow('Clinical0', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ;
    R{C1_Prev0,1}=StateIDRowIndex;          R{C1_Prev0,3}=4;
    R{C1_CFR,1}=StateIDRowIndex;          R{C1_CFR,3}=6;
    R{C1_Progr,1}=StateIDRowIndex;          R{C1_Progr,3}=9;

    %clinical 1
    StateIDRowIndex = findRow('Clinical1', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ;
    R{C2_Prev0,1}=StateIDRowIndex;          R{C2_Prev0,3}=4;
    R{C2_CFR,1}=StateIDRowIndex;          R{C2_CFR,3}=6;
    R{C2_Progr,1}=StateIDRowIndex;          R{C2_Progr,3}=9;

    %clinical 2
    StateIDRowIndex = findRow('Clinical2', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ;
    R{C3_Prev0,1}=StateIDRowIndex;          R{C3_Prev0,3}=4;
    R{C3_CFR,1}=StateIDRowIndex;          R{C3_CFR,3}=6;
    R{C3_Progr,1}=StateIDRowIndex;          R{C3_Progr,3}=9;

    %clinical 3
    StateIDRowIndex = findRow('Clinical3', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ;
    R{C4_Prev0,1}=StateIDRowIndex;          R{C4_Prev0,3}=4;
    R{C4_CFR,1}=StateIDRowIndex;          R{C4_CFR,3}=6;
    R{C4_Progr,1}=StateIDRowIndex;          R{C4_Progr,3}=9;

    %clinical 4
    StateIDRowIndex = findRow('Clinical4', StateIDColIndex, TEMPLATE) + FirstDataRowIndex  ;
    R{C5_Prev0,1}=StateIDRowIndex;          R{C5_Prev0,3}=4;
    R{C5_CFR,1}=StateIDRowIndex;          R{C5_CFR,3}=6;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    R{I1_CFR,1}=findRow('TreatmentStage0', StateIDColIndex, TEMPLATE) + FirstDataRowIndex;         R{I1_CFR,3}=7;R{I1_CFR,4}=14;
    R{I2_CFR,1}=findRow('TreatmentStage1', StateIDColIndex, TEMPLATE) + FirstDataRowIndex;         R{I2_CFR,3}=7;R{I2_CFR,4}=14;
    R{I3_CFR,1}=findRow('TreatmentStage2', StateIDColIndex, TEMPLATE) + FirstDataRowIndex;         R{I3_CFR,3}=7;R{I3_CFR,4}=14;
    R{I4_CFR,1}=findRow('TreatmentStage3', StateIDColIndex, TEMPLATE) + FirstDataRowIndex;         R{I4_CFR,3}=7;R{I4_CFR,4}=14;
    R{I5_CFR,1}=findRow('TreatmentStage4', StateIDColIndex, TEMPLATE) + FirstDataRowIndex;         R{I5_CFR,3}=7;R{I5_CFR,4}=14;

    R{I6_CFR_Function,1}=findRow('TreatPalBasic', StateIDColIndex, TEMPLATE) + FirstDataRowIndex;         R{I6_CFR_Function,3}=7;R{I6_CFR_Function,4}=14;
    R{I7_CFR_Function,1}=findRow('TreatPalExtensive', StateIDColIndex, TEMPLATE) + FirstDataRowIndex;         R{I7_CFR_Function,3}=7;R{I7_CFR_Function,4}=14;

    R{I8_Function,1}=findRow('PalliativeCareBasic', StateIDColIndex, TEMPLATE) + FirstDataRowIndex;         R{I8_Function,4}=7;
    R{I9_Function,1}=findRow('PalliativeCareExtensive', StateIDColIndex, TEMPLATE) + FirstDataRowIndex;         R{I9_Function,4}=7;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     R{Tx_C9,1}=559;R{Tx_C9,2}=559;R{Tx_C9,3}=559;R{Tx_C9,4}=8;R{Tx_C9,5}=9;R{Tx_C9,6}=15;R{Tx_C9,7}=16;
    ...
    for I_loop=1:12
        ...
        %Vaccinated
       % lTEMPLATE{ R{Vacc_inc1618,1}+I_loop-1 ,R{Vacc_inc1618,3} }=0;%HPVonsetG(I_loop,1);
        lTEMPLATE{ R{Vacc_incH,1}+I_loop-1 ,R{Vacc_incH,3} }=0;%HPVonsetG(I_loop,2);
        lTEMPLATE{ R{Vacc_incL,1}+I_loop-1 ,R{Vacc_incL,3} }=0;%HPVonsetG(I_loop,3);
...
function rowID =findRow(stateID,StateIDColIndex, TEMPLATE)
    
    index = zeros(size(TEMPLATE,1),1);
    for i = 1:size(TEMPLATE,1)
    k1 = strcmp(TEMPLATE{i,StateIDColIndex},stateID);
      if(k1 ~= 0 )
        index(i) = i;
      end
    end
